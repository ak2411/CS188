...
def fillYCPT(bayesNet, gameState):
    ...
    yProb = [PROB_BOTH_TOP, PROB_BOTH_BOTTOM, PROB_ONLY_LEFT_TOP, PROB_ONLY_LEFT_BOTTOM]
    #Y_POS_VALS = [BOTH_TOP_VAL, BOTH_BOTTOM_VAL, LEFT_TOP_VAL, LEFT_BOTTOM_VAL]
    "*** YOUR CODE HERE ***"
...
def fillObsCPT(bayesNet, gameState):
    ...
    obsVars = []
    valueMap = {TOP_LEFT_VAL: topLeftPos, TOP_RIGHT_VAL: topRightPos, BOTTOM_LEFT_VAL: bottomLeftPos, BOTTOM_RIGHT_VAL: bottomRightPos}
    "*** YOUR CODE HERE ***"
    #possible house positions
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            ...
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                prob, altered = 0, False
                if obsPos in gameState.getHouseWalls(valueMap[assignment[FOOD_HOUSE_VAR]])or (obsPos in gameState.getHouseWalls(valueMap[assignment[FOOD_HOUSE_VAR]]) and assignment[FOOD_HOUSE_VAR] == assignment[GHOST_HOUSE_VAR]):
                    prob = PROB_FOOD_RED
                    altered = True
                elif obsPos in gameState.getHouseWalls(valueMap[assignment[GHOST_HOUSE_VAR]]):
                    prob = PROB_GHOST_RED
                    altered = True
                elif assignment[obsVar] == NO_OBS_VAL and valueMap[assignment[GHOST_HOUSE_VAR]] != housePos and valueMap[assignment[FOOD_HOUSE_VAR]] != housePos:
                    #none
                    prob = 1

                if assignment[obsVar] == BLUE_OBS_VAL and altered:
                    prob = 1 - prob
                if assignment[obsVar] == NO_OBS_VAL and altered:
                    prob = 0
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    prob = inference.inferenceByVariableElimination(bayesNet, FOOD_HOUSE_VAR, evidence, eliminationOrder)
    for assignment in prob.getAllPossibleAssignmentDicts():
        houseDict[assignment[FOOD_HOUSE_VAR]] += prob.getProbability(assignment)
    # get key w/ max val in houseDict
    return {FOOD_HOUSE_VAR: max(houseDict.iterkeys(), key=(lambda key: houseDict[key]))}
    # util.raiseNotDefined()
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        infFactor = inference.inferenceByVariableElimination(self.bayesNet, HOUSE_VARS, evidence, eliminationOrder)

        for assignment in infFactor.getAllPossibleAssignmentDicts():
            prob = infFactor.getProbability(assignment)
